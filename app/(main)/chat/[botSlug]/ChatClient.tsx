    ...
    const loadChatHistory = async () => {
      ...
      try {
        ...
        if (formattedMessages.length === 0) {
          const welcomeMessage = await ChatService.initializeChat(
            botSlug,
            user.id
          );
          if (welcomeMessage) {
    ...
    <div className="flex-1 flex flex-col min-h-0">
      ...
      <div className="px-4 pb-4 pt-4 flex-1 overflow-y-auto relative z-10 flex flex-col space-y-3">
        {messages.map((message, index) => {
          ...
          const showDayIndicator =
            !previousMessage ||
            isDifferentDay(message.timestamp, previousMessage.timestamp);
                  ...
                  <div className="text-sm prose prose-sm max-w-none">
                    <ReactMarkdown
                      ...
                      components={{
                        ...
                        ul: ({ children }) => (
                          <ul className="list-disc list-outside mb-2 space-y-1 pl-4">
                            {children}
                          </ul>
                        ),
                        ol: ({ children }) => (
                  ...
                  {streamingMessage ? (
                    <div>
                      <div className="text-xs text-gray-400 mb-1">
                        Streaming ({streamingMessage.length} chars):
                      </div>
      ...
      {/* Chat Input */}
      <div className="bg-white/95 backdrop-blur-sm border-t border-gray-200 p-4 relative z-20 flex-shrink-0 pb-safe">
