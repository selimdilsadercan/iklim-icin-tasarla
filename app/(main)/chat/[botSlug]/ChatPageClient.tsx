...
export default function ChatPageClient({ botSlug }: ChatPageClientProps) {
  ...
  return (
    <div
      className="h-[100svh] w-full bg-gray-50 flex flex-col relative overflow-hidden"
      style={{ minHeight: "100svh" }}
    >
      <ChatPatternBackground botSlug={botSlug} />
      {/* Chat Header */}
      <div className="bg-white/95 backdrop-blur-sm border-b border-gray-200 px-4 py-3 relative z-20 flex-shrink-0">
        <div className="flex items-center gap-3">
          <BackButton />
          <div className="w-10 h-10">{botInfo.icon}</div>
          <div className="flex-1">
            <h1 className="font-semibold text-gray-800 -mb-1 mt-0.5">
              {botInfo.name}
            </h1>
...
function getBotInfo(botSlug: string, t: any) {
  ...
  if (persona) {
    ...
    return (
      botMappings[botSlug as keyof typeof botMappings] || botMappings.yaprak
    );
  }
